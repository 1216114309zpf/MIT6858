...
//add two parameters to this function: reqlength,envlength
const char *http_request_line(int fd, char *reqpath,int reqlength, char *env,int envlength, size_t *env_len)
{
    static char buf[8192];      /* static variables are not on the stack */
    char *sp1, *sp2, *qp, *envp = env;
    int lenleft = envlength; 
    ...
    *sp2 = '\0';
    ...
    int len = snprintf(envp,lenleft,"REQUEST_METHOD=%s", buf);
    envp += (len+1);
    lenleft -=(len+1);
    ...
    len = snprintf(envp,lenleft, "SERVER_PROTOCOL=%s", sp2);
    envp += (len+1);
    lenleft -=(len+1);
    ...
    if ((qp = strchr(sp1, '?')))
    {
        *qp = '\0';
        ...
        len = snprintf(envp,lenleft,"QUERY_STRING=%s", qp + 1);
        envp +=(len+1);
        lenleft -=(len+1);
    }

    /* decode URL escape sequences in the requested path into reqpath */
    if(!url_decode(reqpath, sp1,reqlength))
        return "Request path too long";

    if(lenleft <= 0)
            return "Too long request";
    len = snprintf(envp,lenleft,"REQUEST_URI=%s", reqpath);
    envp += (len+1);
    lenleft -= (len+1);

    if(lenleft <= 0)
            return "Too long request";
    len = snprintf(envp,lenleft,"SERVER_NAME=zoobar.org");
    envp += (len+1);
    lenleft -= (len+1);
...
const char *http_request_headers(int fd)
{
    ...
    for (;;)
    {
        ...
        *colon = '\0';
        ...
        if(!url_decode(value, sp,sizeof(value)))
...
void http_serve(int fd, const char *name)
{
    ...
    //use strncat to prevent from buffer overflow
...
//add a new argument "dstlength"
int url_decode(char *dst, const char *src,int dstlength)
{
    int current=0;
    for (;;)
    {
