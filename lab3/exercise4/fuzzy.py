...
class concolic_str(str):
  ...
  def __len__(self):
     ...
     return concolic_int(sym_length(self.__sym),res)
...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    for c in cur_path_constr:
        new_constr = []
        new_constr.append(sym_not(c))
        for constr in cur_path_constr:
            if cur_path_constr.index(c)==cur_path_constr.index(constr):
                break
            new_constr.append(constr)
        new_constr = sym_and(*new_constr)
        if new_constr in checked:
            continue
        else:
            checked.add(new_constr)
        
        (ok, model) = fork_and_check(new_constr)
        if ok == z3.sat:
           new_values = {}
           for k in concrete_values:
              if k in model:
                 ...
                 new_values[k] = concrete_values[k]
           inputs.add(new_values, cur_path_constr_callers[cur_path_constr.index(c)])
    ##
